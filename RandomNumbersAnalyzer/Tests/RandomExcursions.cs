namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class RandomExcursions : Test
    {
        ...
        public override double[] run(bool printResults)
        {
            ...
            //determine cycles
            int J = 0;
            int[] S_k = new int[this.n];
            S_k[0] = 2 * (int)this.model.epsilon[0] - 1;
            int[] cycle = new int[Math.Max(1000, this.n / 100)];
            for (int i = 1; i < this.n; i++)
            {
                S_k[i] = S_k[i - 1] + 2 * this.model.epsilon[i] - 1;
                if (S_k[i] == 0)
                {
                    J++;
                    if (J > Math.Max(1000, this.n / 100))
                    {
                        if (printResults)
                        {
            ...
            double constraint = Math.Max(0.005 * Math.Pow(this.n, 0.5), 500);
        ...
        {
            return "14: Random Excursions Test to run on " + this.n + " bits";
        }
    }
}
