namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class LongestRunOfOnes : Test
    {
        ...
        public override double[] run(bool printResults)
        {
            ...
            double[] pi;
            int K, M, V0;
            this.init(out K, out M, out V0, out pi);  //initialize variables to sensible values

            int[] V = new int[pi.Length];
            int N = this.n / M;    //number of blocks that can be made from the binary string
            for (int i = 0; i < N; i++)
            {
        ...
        {
            return "4: Longest Run Of Ones in a Block Test to run on " + this.n + " bits";
        }
