namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class NonOverlappingTemplateMatching : Test
    {
        ...
        public override double[] run(bool printResults)
        {
            ...
            if (this.B != null)
            { //as we know it is to run on a single test
                mu = (M - this.B.Length + 1) / Math.Pow(2, this.B.Length);
                sigma = M * (1.0 / Math.Pow(2.0, this.B.Length) - (2.0 * this.B.Length - 1.0) / Math.Pow(2.0, 2.0 * this.B.Length));
                return this.run(this.B, printResults);
            }
            ...
            int K = 5;
            mu = (M - this.m + 1) / Math.Pow(2, this.m);
            sigma = M * (1.0 / Math.Pow(2.0, this.m) - (2.0 * this.m - 1.0) / Math.Pow(2.0, 2.0 * this.m));
            ...
            if (printResults)
            {
                ...
                report.Write("-------------------------------------------------------------------------------------");
                report.Write("\tLAMBDA = " + mu + "\tM = " + M + "\tN = " + N + "\tm = " + this.m + "\tn = " + this.n + "");
                report.Write("-------------------------------------------------------------------------------------");
            }

            double[] p_values = new double[numOfTemplates[this.m]];
            try
            {
                using (StreamReader sr = new StreamReader(Assembly.GetExecutingAssembly().GetManifestResourceStream(TEMPLATE_DIR + ".template" + this.m)))
                {
                    for (int i = 0; i < numOfTemplates[this.m]; i++)
                    {
                        ...
                        for (int k = 0; k < this.m; k++)
                        {
                            try
                            {
                                this.B[k] = Convert.ToInt32(numbers[k]); //parse into an array
                            }
                            catch (FormatException)
                            {
                                throw new FormatException("The input data did not consist of a an optional " +
                        ...
                        p_values[i] = this.run(this.B, false)[0];    //get p_value for this template string
        ...
        {
            return "7: Non Overrlapping Template Matching Test to run on " + this.n + " bits compared with a " + this.m + " length templates";
        }

        private double[] run(int[] B, bool printResults)
        {
            if (this.n > this.model.epsilon.Count || this.n <= 0)
            {
            ...
            { //for each start position
                ...
                {    //is there a match for the template
                    bool match = true;
                    for (int k = 0; k < B.Length; k++)
                    {
                        if ((int)B[k] != (int)this.model.epsilon[i * M + j + k])
                        {
                            match = false;
                            break;
                        }
