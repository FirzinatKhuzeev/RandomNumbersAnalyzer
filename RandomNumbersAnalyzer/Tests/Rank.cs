namespace RandomNumbersAnalyzer.Tests {
    ...
    public class Rank : Test {
        ...
        public override double[] run(bool printResults) {
            ...
            int N = this.n / (M * Q); //number of blocks
            int[] Fm = new int[3];
            for (int k = 0; k < N; k++) {
                ...
                for (int i = 0; i < M; i++) {
                    for (int j = 0; j < Q; j++) {
                        matrix[i,j] = this.model.epsilon[k * (M * Q) + j + i * M];
                    }
                }

                int R = this.computeRank(M, Q, matrix); //get the rank of the matrix
            ...
            if (printResults) {
                ...
                report.Write("\t\t(i) NOTE: "+this.n % (M * Q)+" BITS WERE DISCARDED.");
                report.Write("\t\t---------------------------------------------");
        ...
        public override string ToString() {
            return "5: Binary Matrix Rank Test to run on " + this.n + " bits";
        }
        ...
        private int computeRank(int M, int Q, int[,] matrix) {
            ...
            for (int i = 0; i < Math.Min(M,Q) - 1; i++) {
                if (matrix[i, i] == 1) {
                    this.performElementaryRowOperations(true, i, M, Q, ref matrix);
                } else { 	/* matrix[i][i] = 0 */
                    if (this.findUnitElementAndSwap(true, i, M, Q, matrix)) {
                        this.performElementaryRowOperations(true, i, M, Q, ref matrix);
                    }
                }
            }
            ...
            for (int i = Math.Min(M,Q) - 1; i > 0; i--) {
                if (matrix[i, i] == 1) {
                    this.performElementaryRowOperations(false, i, M, Q, ref matrix);
                } else { 	/* matrix[i][i] = 0 */
                    if (this.findUnitElementAndSwap(false, i, M, Q, matrix)) {
                        this.performElementaryRowOperations(false, i, M, Q, ref matrix);
                    }
                }
            }

            return this.determineRank(Math.Min(Q,M), M, Q, matrix);
        }
