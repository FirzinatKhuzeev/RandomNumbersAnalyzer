namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class Frequency : Test
    {
        ...
        public override double[] run(bool printResults)
        {
            ...
            double S_n = this.model.epsilon.GetRange(0, this.n).Sum(delegate (int i) { return 2 * i - 1; });

            //calculate p_value
            double S_obs = Math.Abs(S_n) / Math.Sqrt(this.n);
            ...
            if (printResults)
            {
                ...
                report.Write("\t\t(b) S_n/n               = " + S_n / this.n);
                report.Write("\t\t---------------------------------------------");
        ...
        {
            return "1: Frequency Monobit Test to run on " + this.n + " bits";
        }
    }
}