namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class Universal : Test
    {
            ...
            : base(ref model)
        {
            ...
            else if (n >= 496435200) this.L = 15;
            else if (n >= 231669760) this.L = 14;
            else if (n >= 107560960) this.L = 13;
            else if (n >= 49643520) this.L = 12;
            else if (n >= 22753280) this.L = 11;
            else if (n >= 10342400) this.L = 10;
            else if (n >= 4654080) this.L = 9;
            else if (n >= 2068480) this.L = 8;
            else if (n >= 904960) this.L = 7;
            else if (n >= 387840) this.L = 6;
            else this.L = 5;
        ...
        public override double[] run(bool printResults)
        {
            ...
            int K = (int)(this.n / this.L - (double)this.Q); //number of test blocks
            ...
            for (int i = 1; i <= this.Q; i++)
            {
                ...
                {
                    decValue += this.model.epsilon[(i - 1) * this.L + j] * (long)Math.Pow(2, this.L - 1 - j);  //calculate decimal value of segment
                }
                T[decValue] = i;
            }
            ...
            //test segment
            double sum = 0;
            for (int i = this.Q + 1; i <= this.Q + K; i++)
            {
                long decValue = 0;
                for (int j = 0; j < this.L; j++)
                {
            ...
            double c = 0.7 - 0.8 / (double)this.L + (4 + 32 / (double)this.L) * Math.Pow(K, -3 / (double)this.L) / 15;
            double sigma = c * Math.Sqrt(variance[this.L] / (double)K);

            //calculate p_value
            double arg = Math.Abs(phi - expected_value[this.L]) / (Math.Sqrt(2) * sigma);
            ...
            if (printResults)
            {
                ...
                report.Write("\t\t--------------------------------------------");
                report.Write("\t\t(a) L         = " + this.L);
                report.Write("\t\t(b) Q         = " + this.Q);
                report.Write("\t\t(c) K         = " + K);
                report.Write("\t\t(d) sum       = " + sum);
                report.Write("\t\t(f) variance  = " + variance[this.L]);
                report.Write("\t\t(g) exp_value = " + expected_value[this.L]);
                report.Write("\t\t(h) phi       = " + phi);
                report.Write("\t\t(i) WARNING:  " + (this.n - (this.Q + K) * this.L) + " bits were discarded.");
                report.Write("\t\t-----------------------------------------");
        ...
        {
            return "9: Maurers Universal Statistical Test to run on " + this.n + " bits, with a block length of " + this.L + " and a " + this.Q + " block init sequence";
        }
