namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class CumulativeSums : Test
    {
        ...
        public override double[] run(bool printResults)
        {
            ...
            //compute p_value
            double sum1 = 0.0;
            for (int k = (-this.n / z + 1) / 4; k <= (this.n / z - 1) / 4; k++)
            {
                sum1 += Cephes.normal(((4 * k + 1) * z) / Math.Sqrt(this.n));
                sum1 -= Cephes.normal(((4 * k - 1) * z) / Math.Sqrt(this.n));
            }
            double sum2 = 0.0;
            for (int k = (-this.n / z - 3) / 4; k <= (this.n / z - 1) / 4; k++)
            {
                sum2 += Cephes.normal(((4 * k + 3) * z) / Math.Sqrt(this.n));
                sum2 -= Cephes.normal(((4 * k + 1) * z) / Math.Sqrt(this.n));
            }
            ...
            if (printResults)
            {
                ...
                report.Write("\t\t      CUMULATIVE SUMS (" + (this.mode ? "FORWARD" : "REVERSE") + ") TEST");
                report.Write("\t\t-------------------------------------------");
        ...
        {
            return "13: Cumulative Sums (Cusum) Test to run " + (this.mode ? "forwards" : "backwards") + " on " + this.n + " bits";
        }
