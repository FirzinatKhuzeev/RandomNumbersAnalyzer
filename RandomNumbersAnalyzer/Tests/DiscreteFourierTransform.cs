namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class DiscreteFourierTransform : Test
    {
        ...
        public unsafe override double[] run(bool printResults)
        {
            ...
            {
                X[i] = 2 * (int)this.model.epsilon[i] - 1; //map all bits in binary string (0->-1,1->1)
            }
            double[] m = new double[this.n / 2 + 1];
            //generate DFT of the binary string, using the unsafe functions implemented in C
            fixed (double* wsavePtr = &(new double[2 * this.n])[0])
            {
                fixed (int* ifacPtr = &(new int[15])[0])
                {
                    fixed (double* mPtr = &m[0])
                    {
                        ...
                        {
                            FFT.__ogg_fdrffti(this.n, wsavePtr, ifacPtr);		//init stage for work arrays
                            FFT.__ogg_fdrfftf(this.n, XPtr, wsavePtr, ifacPtr);  //apply FFT on data
                        }
                    }
                }
            }
            ...
            int N_l = 0;
            double T = Math.Sqrt(2.995732274 * this.n); //calculate upper bound (T) (the 95% peak height threshold)
            ...
            double N_0 = 0.95 * this.n / 2.0; //expected number of peaks
            double d = (N_l - N_0) / Math.Sqrt(this.n / 4.0 * 0.95 * 0.05);
        ...
        {
            return "6: Discrete Fourier Transform (Spectral) Test to run on " + this.n + " bits";
        }
    }
}
