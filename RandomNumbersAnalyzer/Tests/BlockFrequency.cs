namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class BlockFrequency : Test
    {
        ...
        public override double[] run(bool printResults)
        {
            ...
            int N = this.n / this.M; 	//number of blocks that can be made from the binary string

            //for each block calculate the proportion of ones
            double sum = 0.0;
            for (int i = 0; i < N; i++)
            {
                double pi = (double)this.model.epsilon.GetRange(i * this.M, this.M).Sum() / (double)this.M;   //sum of bits in block / length of the block
                sum += Math.Pow((pi - 0.5), 2);
            }

            //calculate p_value
            double chi_squared = 4.0 * this.M * sum;
            ...
            if (printResults)
            {
                ...
                report.Write("\t\t(c) block length    = " + this.M);
                report.Write("\t\t(d) Note: " + this.n % this.M + " bits were discarded.");
                report.Write("\t\t---------------------------------------------");
        ...
        {
            return "2: Frequency Test Within a Block to run on " + this.n + " bits and " + this.M + " blocks";
        }
    }
}
