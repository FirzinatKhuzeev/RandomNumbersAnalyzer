namespace RandomNumbersAnalyzer.Tests {
    ...
    public class Runs : Test {
        ...
        public override double[] run(bool printResults) {
            ...
            double pi = (double)this.model.epsilon.GetRange(0, this.n).Sum() / (double)this.n;    //number of ones in the binary string / length

            //prerequisite for test, if not passed test is not applicable   
            if (Math.Abs(pi - 0.5) > (2.0 / Math.Sqrt(this.n))) {
                if (printResults) {
                ...
                for (int i = 1; i < this.n; i++) {
                    if (this.model.epsilon[i] != this.model.epsilon[i - 1]) {
                        V_obs++;
                    }
                }

                //calculate p_value
                double p_value = Cephes.erfc(Math.Abs(V_obs - 2.0 * this.n * pi * (1 - pi)) / (2.0 * pi * (1 - pi) * Math.Sqrt(2 * this.n)));
        ...
        public override string ToString() {
            return "3: Runs Test to run on " + this.n + " bits";
        }
    }
}
