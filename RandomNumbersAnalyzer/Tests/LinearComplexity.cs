namespace RandomNumbersAnalyzer.Tests
{
    ...
    public class LinearComplexity : Test
    {
        ...
        public override double[] run(bool printResults)
        {
            ...
            int N = this.n / this.M;    //number of blocks

            double mu = this.M / 2.0 + (9.0 + Math.Pow(-1, this.M + 1)) / 36.0 - 1.0 / Math.Pow(2, this.M) * (this.M / 3.0 + 2.0 / 9.0);

            double[] v = new double[K + 1];

            for (int i = 0; i < N; i++)
            {
                int[] T = new int[this.M];   //initialize the work arrays
                int[] P = new int[this.M];
                int[] C = new int[this.M];
                ...
                int blockPos = 0;
                while (blockPos < this.M)
                {
                    d = (int)this.model.epsilon[i * this.M + blockPos];
                    for (int j = 1; j <= L; j++)
                    {
                        d += C[j] * this.model.epsilon[i * this.M + blockPos - j];
                    }
                    d = d % 2;
                    if (d == 1)
                    {
                ...
                double Ti = Math.Pow(-1, this.M) * (L - mu) + 2.0 / 9.0;
            ...
            if (printResults)
            {
                ...
                report.Write("-----------------------------------------------------");
                report.Write("\tM (substring length)     = {0}" + this.M);
                ...
                report.Write("-----------------------------------------------------");
                report.Write("\tNote: " + this.n % this.M + " bits were discarded!");
                for (int i = 0; i < K + 1; i++)
                {
        ...
        {
            return "10: Linear Complexity Test to run on " + this.n + " bits, and blocks of size " + this.M;
        }
    }
}
